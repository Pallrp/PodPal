    ...
    return __awaiter(this, void 0, void 0, function* () {
        return new Promise((resolve, reject) => {
            ...
            reader.onload = (event) => {
                var _a;
                try {
                    const csvText = (_a = event.target) === null || _a === void 0 ? void 0 : _a.result;
                    if (!csvText) {
                    ...
                    players.forEach(player => {
                        const powerLevels = [];
                        if (player.powerLevels.cedh)
                            powerLevels.push(Powerlevel.COMP);
                        if (player.powerLevels.high)
                            powerLevels.push(Powerlevel.HIGH);
                        if (player.powerLevels.mid)
                            powerLevels.push(Powerlevel.MEDIUM);
                        if (player.powerLevels.casual)
                            powerLevels.push(Powerlevel.CASUAL);
...
function addPlayerWithId(name, powerLevels) {
...
function addPlayerToList(sourcePlayerId, targetPlayerId, listType) {
    try {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    importInput.addEventListener('change', (event) => __awaiter(this, void 0, void 0, function* () {
        const target = event.target;
        if (target.files && target.files.length > 0) {
            const file = target.files[0];
            yield importPlayersFromCSV(file);
...
//# sourceMappingURL=csv_importer.js.map