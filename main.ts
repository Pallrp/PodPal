...
var savedSolutions: Array<{seatings: Array<Array<number>>, score: number}> = [];
// Track current solution for modifications
var currentSolution: {seatings: Array<Array<number>>, score: number} | null = null;
...
function showFeedbackToast(message: string, isError: boolean = false) {
...
function removePod() {
    if (!currentSolution || currentSolution.seatings.length <= 1) return;
...
function updateSavedSolutionsList() {
    ...
    savedSolutions.forEach((solution, index) => {
        const button = solutionButton.cloneNode(true) as HTMLButtonElement;
...
function stageSavedSolution(index: number) {
...
function updateStagedSolution(solution: {seatings: Array<Array<number>>, score: number}) {
    ...
    solution.seatings.forEach((table, tableIndex) => {
        const tableEl = solutionTableTemplate.cloneNode(true) as HTMLElement;
        ...
        const seatsContainer = tableEl.querySelector('.solution-seats-container') as HTMLElement;
    ...
    if (unsortedPlayers.length > 0) {
        const unsortedTableEl = solutionTableTemplate.cloneNode(true) as HTMLElement;
        ...
        const seatsContainer = unsortedTableEl.querySelector('.solution-seats-container') as HTMLElement;
    ...
    if (remainingEmptyTables > 0) {
        for (let i = 0; i < remainingEmptyTables; i++) {
            const emptyTableEl = solutionTableTemplate.cloneNode(true) as HTMLElement;
            emptyTableEl.id = `empty-table-${i}`;
            emptyTableEl.removeAttribute('class');
            emptyTableEl.classList.add('new-col-3', 'empty-table');
            
            const seatsContainer = emptyTableEl.querySelector('.solution-seats-container') as HTMLElement;
...
function createSeatElement(player: Player) {
    const seatEl = solutionSeatTemplate.cloneNode(true) as HTMLElement;
    seatEl.setAttribute('data-player-id', player.id.toString());
    
    const nameContainer = seatEl.querySelector('.solution-seat-name-container') as HTMLElement;
    const nameSpan = nameContainer.querySelector('.solution-name') as HTMLElement;
    nameSpan.textContent = player.name;
    
    const powerContainer = seatEl.querySelector('.solution-seat-power-container') as HTMLElement;
    player.power.forEach(power => {
        const powerPill = powerPillTemplate.cloneNode(true) as HTMLElement;
...
function handlePlayerDragStart(event: DragEvent) {
    const seat = event.target as HTMLElement;
    const playerId = seat.getAttribute('data-player-id');
    const tableId = seat.closest('.solution-seats-container')?.parentElement?.id.replace('solution-table-', '') || '-1';
...
function handlePlayerDrop(event: DragEvent) {
    ...
    const targetTableId = targetElement.getAttribute('data-table-id') || 
                        targetElement.closest('.solution-seats-container')?.parentElement?.id.replace('solution-table-', '') || 
                        '-1';
...
function movePlayerBetweenTables(playerId: number, sourceTableId: string, targetTableId: string) {
    if (!currentSolution) return;
...
function swapPlayers(player1Id: number, table1Id: string, player2Id: number, table2Id: string) {
    if (!currentSolution) return;
...
function getPlayerIdFromElement(element: HTMLElement): number | null {
...
function showPlayerSelectionDropdown(sourcePlayerEl: HTMLElement, listType: 'blacklist' | 'whitelist') {
    ...
    if (!PLAYERS[sourcePlayerId]) {
        ...
        const playerName = sourcePlayerEl.querySelector('.player-name')?.textContent || 'Unknown';
        const playerPowerEls = sourcePlayerEl.querySelectorAll('.powerpill');
        const playerPowers = new Set<number>();
        ...
        playerPowerEls.forEach(pill => {
            ...
            if (powerClass) {
                if (powerClass === 'power-casual') playerPowers.add(Powerlevel.CASUAL);
                else if (powerClass === 'power-medium') playerPowers.add(Powerlevel.MEDIUM);
                else if (powerClass === 'power-high') playerPowers.add(Powerlevel.HIGH);
                else if (powerClass === 'power-comp') playerPowers.add(Powerlevel.COMP);
            }
        });
        
        if (playerPowers.size === 0) playerPowers.add(Powerlevel.MEDIUM);
    ...
    if (!sourcePlayer.whitelist) sourcePlayer.whitelist = new Set<number>();
    if (!sourcePlayer.blacklist) sourcePlayer.blacklist = new Set<number>();
    ...
    playerElements.forEach(playerEl => {
        const targetPlayerId = getPlayerIdFromElement(playerEl as HTMLElement);
        if (targetPlayerId !== null && targetPlayerId !== sourcePlayerId) {
            const playerName = playerEl.querySelector('.player-name')?.textContent || 'Unknown';
                ...
                option.addEventListener('click', () => {
                    ...
                    if (listType === 'blacklist') {
                        ...
                        if (!PLAYERS[targetPlayerId].blacklist) {
                            PLAYERS[targetPlayerId].blacklist = new Set<number>();
                        }
                        ...
                        if (!PLAYERS[targetPlayerId].whitelist) {
                            PLAYERS[targetPlayerId].whitelist = new Set<number>();
                        }
    ...
    document.addEventListener('click', function closeDropdown(e) {
        if (!dropdown.contains(e.target as Node) && 
            !(e.target as HTMLElement).closest('.blacklistbutton, .whitelistbutton')) {
            dropdown.remove();
...
function updatePlayerListUI(playerId1: number, playerId2: number, listType: 'blacklist' | 'whitelist') {
    const playerEl1 = document.getElementById(`player-${playerId1}`);
    const playerEl2 = document.getElementById(`player-${playerId2}`);
    
    if (!playerEl1 || !playerEl2) return;
    
    // Get the list container for player 1
    const container1 = playerEl1.querySelector(`.${listType}-container .${listType}`) as HTMLElement;
    if (!container1) return;
    
    // Get the list container for player 2
    const container2 = playerEl2.querySelector(`.${listType}-container .${listType}`) as HTMLElement;
    ...
    item1.querySelector('.rm-list-btn')?.addEventListener('click', () => {
        item1.remove();
        item2.remove();
    ...
    // Add remove event listener
    item2.querySelector('.rm-list-btn')?.addEventListener('click', () => {
        item1.remove();
        item2.remove();
    ...
    const container1Parent = playerEl1.querySelector(`.${listType}-container`) as HTMLElement;
    const container2Parent = playerEl2.querySelector(`.${listType}-container`) as HTMLElement;
...
function addPlayer(name:string, powerLevels:Array<number>) : number {
    try {
        let playerEl:HTMLElement = (playerTemplateEl.cloneNode(true) as HTMLElement);
        let playerID = "player-" + String(playerCount);
        playerEl.setAttribute("id", playerID);
        
        let nameEl:(HTMLElement | null) = playerEl.querySelector(".player-name") as HTMLElement;
        if (nameEl) {
            nameEl.innerHTML = name;
        }
        
        let powerContainer:(HTMLElement | null) = playerEl.querySelector(".player-power-container") as HTMLElement;
        if (powerContainer) {
            for (let pow of powerLevels) {
                let powerpill:HTMLElement = (powerPillTemplate.cloneNode(true) as HTMLElement);
        ...
        let powerSet = new Set<number>(powerLevels);
        let player = new Player(playerCount, name, powerSet);
        player.whitelist = new Set<number>();
        player.blacklist = new Set<number>();
        PLAYERS[playerCount] = player;
        
        // Add BL/WL button click handlers directly
        const blacklistBtn = playerEl.querySelector('.blacklistbutton') as HTMLElement;
        const whitelistBtn = playerEl.querySelector('.whitelistbutton') as HTMLElement;
        
        // Fix remove player button
        let rmButton:(HTMLElement | null) = playerEl.querySelector(".rm-player-btn");
        if (rmButton) {
        ...
        blacklistBtn.addEventListener('click', (event) => {
            ...
            const blacklistContainer = playerEl.querySelector('.blacklist-container') as HTMLElement;
        ...
        whitelistBtn.addEventListener('click', (event) => {
            ...
            const whitelistContainer = playerEl.querySelector('.whitelist-container') as HTMLElement;
...
function addListPlayer(
    ...
    let listElement = playerEl1.querySelector("." + list) as HTMLElement;
    if (listElement) {
        ...
        if (playerId1 && playerId2) {
            ...
            let p1Name = playerEl1.querySelector(".player-name")?.textContent || "";
            let p2Name = playerEl2.querySelector(".player-name")?.textContent || "";
            if (!p1Name || !p2Name) {
                return;
            }
            let listTemplate = document.getElementById("list-template") as HTMLElement;
            let listPlayer = listTemplate.cloneNode(true) as HTMLElement;
            ...
            let listPlayerName = listPlayer.querySelector(".player-list-name") as HTMLElement;
            listPlayerName.innerText = p2Name;
            let rmButton = listPlayer.querySelector(".rm-list-btn") as HTMLElement;
            rmButton.addEventListener("click", (ev) => {
                let p2El = document.getElementById(playerId2);
                let p1ListEl = rmButton.closest("[value='" + playerId2 + "']") as HTMLElement;
                if (p2El && p1ListEl) {
                    removeListPlayer(list, playerEl1, p1ListEl, p2El, (p2El.querySelector("." + list) as HTMLElement).querySelector("[value='" + playerId1 + "']") as HTMLElement);
                }
            });
            listElement.appendChild(listPlayer);
            
            // Add player to the opposite player's list
            let list2Element = playerEl2.querySelector("." + list) as HTMLElement;
            let l2Player = listTemplate.cloneNode(true) as HTMLElement;
            
            // Update to use the new player-list-item styling
            l2Player.classList.add('player-list-item');
            
            l2Player.setAttribute("value", playerId1);
            let l2PlayerName = l2Player.querySelector(".player-list-name") as HTMLElement;
            l2PlayerName.innerText = p1Name;
            let rm2Button = l2Player.querySelector(".rm-list-btn") as HTMLElement;
            rm2Button.addEventListener("click", (ev) => {
                let p1El = document.getElementById(playerId1);
                let p2ListEl = rm2Button.closest("[value='" + playerId1 + "']") as HTMLElement;
                if (p1El && p2ListEl) {
                    removeListPlayer(list, playerEl2, p2ListEl, p1El, (p1El.querySelector("." + list) as HTMLElement).querySelector("[value='" + playerId2 + "']") as HTMLElement);
                }
            });
...
function removeListPlayer(
    ...
    const p1Name = pEl1.querySelector(".player-name")?.textContent || "Player 1";
    const p2Name = pEl2.querySelector(".player-name")?.textContent || "Player 2";
    ...
    let player1Id = parseInt(pEl1.getAttribute("id")?.replace("player-", "") || "0");
    let player2Id = parseInt(pEl2.getAttribute("id")?.replace("player-", "") || "0");
    
    if (list === "blacklist") {
        if (PLAYERS[player1Id]) PLAYERS[player1Id].blacklist.delete(player2Id);
        if (PLAYERS[player2Id]) PLAYERS[player2Id].blacklist.delete(player1Id);
        
        // Show feedback
        showFeedbackToast(`Removed ${p2Name} from ${p1Name}'s blacklist`, false);
    } else {
        if (PLAYERS[player1Id]) PLAYERS[player1Id].whitelist.delete(player2Id);
        if (PLAYERS[player2Id]) PLAYERS[player2Id].whitelist.delete(player1Id);
...
function newSearch(agent:string) : void {
    ...
    Array.from(searchBtns).forEach((btn:Element) => {
    ...
    setTimeout(() => {
        try {
            ...
            // Show error message
            showFeedbackToast('Search failed: ' + (error as Error).message, true);
        }
    }, 50);
}
...
function setupConfigurationHandlers() : void {
    ...
    const weightInputs = {
        powerImbalance: document.getElementById('powerImbalanceWeight') as HTMLInputElement,
        powerDiversity: document.getElementById('powerDiversityWeight') as HTMLInputElement,
        powerDiff: document.getElementById('powerDiffWeight') as HTMLInputElement,
        blacklist: document.getElementById('blacklistWeight') as HTMLInputElement,
        emptySeat: document.getElementById('emptySeatWeight') as HTMLInputElement,
        unseated: document.getElementById('unseatedWeight') as HTMLInputElement,
        playHistory: document.getElementById('playHistoryWeight') as HTMLInputElement
    };

    // Setup weight value displays
    const weightValues = {
        powerImbalance: document.getElementById('powerImbalanceValue') as HTMLElement,
        powerDiversity: document.getElementById('powerDiversityValue') as HTMLElement,
        powerDiff: document.getElementById('powerDiffValue') as HTMLElement,
        blacklist: document.getElementById('blacklistValue') as HTMLElement,
        emptySeat: document.getElementById('emptySeatValue') as HTMLElement,
        unseated: document.getElementById('unseatedValue') as HTMLElement,
        playHistory: document.getElementById('playHistoryValue') as HTMLElement
    };
...
function addButtonEvents() : void {
    addPlayerButton.addEventListener("click", (ev) => {
        let playerName: string = playerNameInput.value.trim();
        ...
        document.querySelectorAll('#power-level-checkboxes input[type="checkbox"]:checked').forEach(checkbox => {
            const powerLevel = parseInt((checkbox as HTMLInputElement).value);
            if (!isNaN(powerLevel)) {
...
function getHighestPowerLevel(powerContainer: Element | null): number {
    if (!powerContainer) return 0;
...
function setupSearchProgressListeners() {
    ...
    document.addEventListener('searchProgressUpdate', (event: CustomEvent) => {
    ...
    document.addEventListener('searchComplete', (event: CustomEvent) => {
...
function getPlayCount(playerId1: number, playerId2: number): number {
...
function addPlayHistoryIndicator(seat: HTMLElement, playCount: number) {
...
function createPlayerConnection(seat1: HTMLElement, seat2: HTMLElement, playCount: number) {
...
function showWelcomeMessage() {
    ...
    welcomeToast.querySelector('.close-btn')?.addEventListener('click', () => {
...
function showSearchOptimizationTips() {
    ...
    // Add close button functionality
    tipsToast.querySelector('.close-btn')?.addEventListener('click', () => {
...
function makePowerPillsEditable() {
    ...
    document.addEventListener('click', (event) => {
        ...
        if (target.classList.contains('powerpill') && target.closest('.player-power-container')) {
            ...
            const playerContainer = target.closest('.player-container-instance') as HTMLElement;
            ...
            if (!editor) {
                ...
                updatePowerPillEditor(editor as HTMLElement, playerId);
            }
...
function createPowerPillEditor(playerId: number): HTMLElement {
...
function updatePowerPillEditor(editor: HTMLElement, playerId: number): void {
    editor.setAttribute('data-player-id', playerId.toString());
...
function togglePlayerPowerLevel(playerId: number, powerLevel: number): void {
    // Get the player
    const player = PLAYERS[playerId];
    if (!player) return;
    ...
    const powerContainer = playerEl.querySelector('.player-power-container') as HTMLElement;
...
function updatePlayerPowerPills(playerEl: HTMLElement, player: Player): void {
    const powerContainer = playerEl.querySelector('.player-power-container') as HTMLElement;
    if (!powerContainer) return;
